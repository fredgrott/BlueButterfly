...
import com.github.shareme.bluebutterfly.core.internal.SimpleTextWatcher;
import com.github.shareme.bluebutterfly.core.recycler.Row;
...
public class AutoCompleteTextView extends EditText implements TouchMarginView, AnimatedView {
    ...
    List<FilteringResult> filteredItems = new ArrayList<>();

    public List<FilteringResult> filter(Word word) {
        filteredItems.clear();
        if (word.length() == 0) {
        ...
        for (int i = 0; i < dataProvider.getItemCount(); i++) {
            ...
            if (item.indexOf(text) == 0 && word.postCursor.length() == 0) {
                ...
                spannable.setSpan(new HintSpan(getCurrentHintTextColor()), text.length(), item.length(),
                        Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                filteredItems.add(new FilteringResult(AutoCompleteTextView.FILTERING_START, spannable));
            } else {
                Spannable spannable = partialMatch(item, word);
                if (spannable != null) {
                    filteredItems.add(new FilteringResult(AutoCompleteTextView.FILTERING_PARTIAL, spannable));
                }
            }
        }
    ...
    private Spannable partialMatch(String item, Word word) {  // item: 'lemon', text: 'le'
        Spannable spannable = new SpannableStringBuilder(item);
        int i = 0, j = 0;
        ...
        for (; i < item.length() && j < text.length(); i++) {
            if (item.charAt(i) == text.charAt(j)) {
                ...
                spannable.setSpan(new HintSpan(getCurrentHintTextColor()), i, i + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        spannable.setSpan(new HintSpan(getCurrentHintTextColor()), i, item.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        if (j == text.length()) {
            return spannable;
        }
        return null;
    ...
    static class AutoCompleteRow implements Row<FilteringResult> {
        ...
        private final com.github.shareme.bluebutterfly.core.widget.TextView text;
        ...
        public AutoCompleteRow(ViewGroup parent) {
            ...
            text = (com.github.shareme.bluebutterfly.core.widget.TextView) view.findViewById(R.id.carbon_autoCompleteLayoutRowText);
        }
