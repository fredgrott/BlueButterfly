 ...
 * setting your values with {@link #setupInnerViewElements(ViewGroup, View)} method.
 * </p>
 * Usage:
 * <pre><code>
 *
...
public class Card extends BaseCard {
    ...
    public boolean isClickable() {
        if (mIsClickable) {
            if (mOnClickListener == null && (mMultipleOnClickListener == null || mMultipleOnClickListener.isEmpty())) {
                Timber.w(TAG, "Clickable set to true without onClickListener");
                return false;
            }
        }
        return mIsClickable;
    ...
    public boolean isLongClickable() {
        if (mOnLongClickListener == null) {
            if (mIsLongClickable)
                Timber.w(TAG, "LongClickable set to true without onLongClickListener");
            return false;
        }
    ...
    public void addPartialOnClickListener(int area, OnCardClickListener onClickListener) {
        ...
        if (area < CLICK_LISTENER_ALL_VIEW && area > CLICK_LISTENER_CONTENT_VIEW)
            Timber.w(TAG, "area value not valid in addPartialOnClickListner");
